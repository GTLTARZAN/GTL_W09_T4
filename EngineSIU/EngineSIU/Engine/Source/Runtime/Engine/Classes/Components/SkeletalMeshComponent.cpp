...
void USkeletalMeshComponent::UpdateBoneGlobalPose()
{
    ...
    {
        if (Bones[i].ParentIndex == 0xFFFF)
        {
            RecursiveUpdateGlobal(i);
            break;
        }
    }
}

void USkeletalMeshComponent::RotateBone(int BoneIndex, FVector Rot)
{
    TArray<FBone>& Bones = SkeletalMesh->GetRenderData()->Skeleton.Bones;
...
void USkeletalMeshComponent::RecursiveUpdateGlobal(int Index)
{
    ...
    {
        RecursiveUpdateGlobal(ChildIndex);
    }
}
...
void USkeletalMeshComponent::SetProperties(const TMap<FString, FString>& InProperties)
{
    ...
    if (TempStr) // 키가 존재하는지 확인
    {
        ...
        {
            // 경로 문자열로 USkeletalMesh 에셋 로드 시도
    ...
    else // 키 자체가 없으면
    {
        ...
        UE_LOG(LogLevel::Display, TEXT("SkeletalMesh key not found for %s, mesh unchanged."), *GetName());
    }
}
...
TArray<FName> USkeletalMeshComponent::GetMaterialSlotNames() const
{
    ...
    for (const FMaterial* Material : SkeletalMesh->GetMaterials())
    {
